...
setwd("/Users/bryansmacbookpro/Documents")
getwd()

## Load the `data/r4ds/heights.csv` to
heights_df <- read.csv("Downloads/heights.csv")
...
## `height` vs. `earn`
ggplot(heights_df, aes(x=height, y=earn)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
## `age` vs. `earn`
ggplot(heights_df, aes(x=age, y=earn)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
## `ed` vs. `earn`
ggplot(heights_df, aes(x=ed, y=earn)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
