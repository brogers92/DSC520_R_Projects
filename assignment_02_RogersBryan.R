...
file_path <- "data/G04ResultsDetail2004-11-02.xls"
...
voter_turnout_df1 <- read_excel(file_path, sheet = "Voter Turnout", skip = 1)
...
col_names("ward_precint", "ballots_cast", "registered_voters", "voter_turnout")
voter_turnout_df2 <- read_excel(file_path, sheet = "Voter Turnout", skip = 2, col_names = col_names)
...
db_path <- "data/tidynomicon/example.db"
...
person_df <- dbGetQuery(db, sql_query)
head(person_df)
